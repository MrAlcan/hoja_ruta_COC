...
CREATE VIEW `documentos_procedencia`
	AS SELECT prc`codigo_hoja_ruta`, prc`solicitante`, prc`descripcion_solicitud`, areas`nombre_area`, us`nombre_usuario`
    FROM `flujo_procedimiento` AS fp INNER JOIN `procedimiento` AS prc ON fp.id_procedimiento_flujo = prc.id_procedimiento
    INNER JOIN `areas` ON fp.id_area_procedencia = areas.id_area
    INNER JOIN `usuarios` AS us ON fp.id_usuario_envia = us.ci;
...
CREATE VIEW `documentos_procedencia`
	AS SELECT procedimiento.`codigo_hoja_ruta`, procedimiento.`solicitante`, procedimiento.`descripcion_solicitud`, areas.`nombre_area`, usuarios.`nombre_usuario`
...
CREATE VIEW `documentos_procedencia`
    ...
    FROM `flujos_procedimientos` INNER JOIN `procedimiento` ON flujo_procedimiento.id_procedimiento_flujo = procedimiento.id_procedimiento
    INNER JOIN `areas` ON flujo_procedimiento.id_area_procedencia = areas.id_area
    INNER JOIN `tipo_procedimiento` ON procedimiento.id_tipo_procedimiento_realizado = tipo_procedimiento.id_tipo_procedimiento
    INNER JOIN `flujo_procedimiento` ON flujo_procedimiento.id_flujo = flujos_procedimientos.id_flujo_prc
    INNER JOIN `usuarios` ON flujo_procedimiento.id_usuario_envia = usuarios.ci;
...
CREATE VIEW `documentos_destino`
    ...
    INNER JOIN `usuarios` ON flujo_procedimiento.id_usuario_envia = usuarios.ci
    INNER JOIN `tipo_procedimiento` ON procedimiento.id_tipo_procedimiento_realizado = tipo_procedimiento.id_tipo_procedimiento
    GROUP BY flujo_procedimiento.`id_procedimiento_flujo` DESC;
